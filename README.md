...
2. perform the data preprocessing on the full_jcvi_rxn_list.csv by making use of data_preprocessing.py
3. Add the output files from the data preprocessing, together with updated smiles.csv to the same directory and run the reverse simulation script
...
All dependancies for this program can be found in the requirements file. The files in main_15000_4000 is used to run a simulation for 15 000 systems over 4 000 iterations and the files in main_60000_8000 to run a simulation for 60 000 systems over 8 000 iterations. 

#### Running instructions

This simulation is very computationally expensive and requires the use of a high perfomance computing (HPC) unit/cluster.

1. Use the same "chem_dict.txt" and "rxn numbers.csv" files from the reverse simulation produced by the data preprocessing script
2. Create a new directory and add the files in HPC setup files and Main simulation
3. Change the PBS directives in the simulation file to fit the capacity of the HPC you are using and then submit the script
...
The network visualization can be done by running the "vis1.pbs" script in Data visualization and analysis. The fitness and standard deviation graphs can be generated by running the "vis1.pbs" script in Fitness Calculation. 

## Data and dependencies

### Data sources 

The data used to compile the JCVI reaction list was obtained from:
...
### Environment
The virtual environment is automatically setup when running the simulation script. 
It makes use of the requirements file together with the "source sim2env/bin/activate" command. 
