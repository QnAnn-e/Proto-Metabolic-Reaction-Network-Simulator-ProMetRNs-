# all the steps that comes before
import pandas as pd
import re
...
# getting the smiles id's of the metabolites to use in the fitness function
# but this step uses an api and it makes everything take looooong
class Prelim_Fit_Calc():

    def __init__(self):
        pass

    def say_cheese(self, adjusted_met_names):

        """ input: adjusted met names = list of metabolite names with very specific naming conventions
            output: met smiles = list of isomeric smiles id's for the metabolite names provided"""

        # using the metabolite names to retrieve compound id's from pubchem
        met_compounds = []
        met_smiles = []
        for metabolite in adjusted_met_names:
            current_met = pcp.get_compounds(metabolite, 'name')
            met_compounds.append(current_met)

        # but oh no, why does it give more than one cid per compound? #
        # let's keep only the first one because according to the internet it gives the best fit first
        list_of_cids = []
        list_of_compounds = []

        for cids in met_compounds:
            cid = str(cids[0])
            new_cid = re.sub('[^0-9]', '', cid)
            list_of_compounds.append(new_cid)
            name_in_db = pcp.Compound.from_cid(new_cid)
            list_of_cids.append(name_in_db)

        # here we get the smiles from the closest match to our met name
        for ids in list_of_cids:
            met_smile = ids.isomeric_smiles
            met_smiles.append(met_smile)
...
# lists jcvi data
j_reaction_nr, j_reactions = reactions.unpacking_jcvi("full_jcvi_rxn_list.csv")
...
# Class THREE
# glycolysis_fit = Prelim_Fit_Calc()
# metabolites in glycolysis
# future endeavor is to just use the metabollite names generated in the previous steps instead of this list, but
# unfortunately not all metabolites in jcvi have smiles ids :(
"""adjusted_met_names = ['D-GLUCOSE-6-PHOSPHATE', 'D-FRUCTOSE-6-PHOSPHATE', 'FRUCTOSE-1,6-BISPHOSPHATE', 'ADP', 'ATP',
                      'H+',
                      'GLYCERALDEHYDE-3-PHOSPHATE', 'NAD+', 'PHOSPHATE', 'NADH', 'DIHYDROXYACETONE PHOSPHATE',
                      '1,3-BIPHOSPHOGLYCERATE', 'NADP+', 'NADPH', 'H2O', '3-PHOSPHOGLYCERATE', '2-PHOSPHOGLYCERATE',
                      'PHOSPHOENOLPYRUVATE', 'PYRUVATE']"""
# getting the smiles for the mets in glyc
#met_smiles = glycolysis_fit.say_cheese(adjusted_met_names)
# optional rewrite to a file if i need to include as supplementary material
# with open('glyc_metabolite_smiles.txt', 'w') as f:
#     for item in met_smiles:
#         f.write(item + "\n")